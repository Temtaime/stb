...
final class MySQL
{
	this(string host, string user, string pass, string db, uint port = 3306)
	{
		...
		mysql_real_connect(_db, host.toStringz, user.toStringz, pass.toStringz, db.toStringz, port, null, 0) || throwError(lastError);
	}
...
package:
	...
	auto process(A...)(MYSQL_STMT* stmt)
	{
		...
		{
			bool attr = true;
			!mysql_stmt_attr_set(stmt, STMT_ATTR_UPDATE_MAX_LENGTH, &attr) || throwError(lastError(stmt));
		}
