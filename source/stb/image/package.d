...
import etc.c.zlib, stb;
import std.exception, std.range, std.mmfile, std.file, std.path, std.utf, std.string;

import core.stdc.string : memcpy;
import std.algorithm : max, swap, filter, each;
...
final class Image
{
	...
	this(in void[] data)
	{
		auto p = cast(Color*)stbi_load_from_memory(cast(const(ubyte)*)data.ptr, cast(uint)data.length, cast(int*)&_w, cast(int*)&_h, null, 4);
